...
# ============================================================
# CSS — CONTRASTE, CORES E RESPONSIVIDADE
# ============================================================
...
st.markdown(
    ...
    <style>
        ...
        [data-testid="stHeader"] {
            background: rgba(248, 252, 253, 0.90) !important;
        }
        ...
        div[data-baseweb="input"]:focus-within {
            ...
            box-shadow:
                0 0 0 3px rgba(4, 127, 158, 0.14) !important;
        }
...
def texto_seguro(valor):
    """Escapa textos inseridos em blocos HTML."""
    return html.escape(str(valor).strip())


def nome_arquivo_seguro(texto):
    """Gera nome de arquivo sem caracteres incompatíveis."""
...
def obter_nivel(percentual):
    """Retorna o nível geral de maturidade."""
    if percentual < 20:
        return (
            "Crítico",
...
def obter_prioridade(percentual, avaliadas):
    """Retorna a prioridade de tratamento."""
    if avaliadas == 0:
        return "Não avaliada"
...
def recomendacao_dimensao(codigo):
    """Retorna recomendação correspondente à dimensão."""
    recomendacoes = {
        "GV": (
    ...
    return recomendacoes.get(codigo, "Revisar os controles da dimensão.")
...
def criar_grafico_colunas(resultados):
    """Cria gráfico de colunas de maturidade por dimensão."""
    nomes = [resultado["dimensao"] for resultado in resultados]
    percentuais = [
    ...
    eixo.set_ylabel("Maturidade (%)", color="#102F3B")
    eixo.set_ylim(0, 110)
...
def criar_grafico_pizza(resultados):
    """Cria gráfico de pizza com a distribuição dos pontos."""
...
def figura_para_png(figura, dpi=180):
    """Converte gráfico Matplotlib em imagem PNG."""
    imagem = io.BytesIO()
...
def montar_csv_completo(
    ...
    """Gera CSV completo compatível com Excel."""
    arquivo = io.StringIO()
    ...
    escritor.writerow(
        ["Pontuação geral", f"{pontos_totais}/{maximo_total}"]
    )
    escritor.writerow([])
    ...
    for resposta in respostas_detalhadas:
        ...
        escritor.writerow(
            [
                ...
                "Avaliado" if pontuacao is not None else "Não avaliado",
            ]
        )
...
def adicionar_rodape_pdf(canvas, documento):
    """Adiciona rodapé e número da página."""
    canvas.saveState()

    largura, _ = landscape(A4)

    canvas.setStrokeColor(colors.HexColor("#BFD8E1"))
    canvas.line(
        1.2 * cm,
        1.05 * cm,
        largura - 1.2 * cm,
    ...
    canvas.setFillColor(colors.HexColor("#496570"))
...
def montar_pdf(
    ...
    """Gera o PDF completo com gráficos e checklist."""
    ...
    elementos = [
        Paragraph("CTR CyberCheck", estilo_titulo),
        Paragraph(
            (
    ...
    elementos.append(
        Paragraph("Resultado por dimensão", estilo_secao)
    )
    ...
    elementos.append(PageBreak())
    elementos.append(
        Paragraph("Checklist completo", estilo_secao)
    )
    ...
    for resposta in respostas_detalhadas:
        ...
        dados_checklist.append(
            [
                ...
                Paragraph(
                    "Avaliado" if avaliada else "Não avaliado",
                    estilo_celula_central,
                ),
            ]
        )
    ...
    elementos.append(Spacer(1, 0.5 * cm))
    elementos.append(
        Paragraph("Observação metodológica", estilo_secao)
    )
    elementos.append(
        Paragraph(
            (
                ...
                "análise de vulnerabilidades, teste de invasão ou avaliação "
                "especializada."
            ),
            estilo_normal,
        )
...
for indice_dimensao, dimensao in enumerate(DIMENSOES, start=1):
    ...
    for inicio in range(0, len(perguntas), 2):
        ...
        for deslocamento, coluna in enumerate(colunas):
            ...
            chave = (
                f'resposta_{dimensao["codigo"]}_{indice_pergunta}'
            )
...
if (
    ...
    figura_colunas_tela = criar_grafico_colunas(resultados)
    figura_pizza_tela = criar_grafico_pizza(resultados)
    ...
    # ========================================================
    # CHECKLIST COMPLETO NA TELA
    # ========================================================

    with st.expander(
        "📋 Visualizar todos os dados do checklist",
        expanded=False,
    ):
        tabela_checklist = pd.DataFrame(
            [
                {
                    "Nº": resposta["numero"],
